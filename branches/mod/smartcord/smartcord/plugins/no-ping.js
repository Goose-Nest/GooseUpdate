...
module.exports = new Plugin({
  ...
  author: "Joe 🎸#7070",
  description:
    "Removes ping box for any server that is muted and has @everyone surpressed.",
  color: "aqua",

  load: async function () {
    while (
      !findModule("getMentionCount", true) ||
      !findModule("isMuted", true) ||
      !findModule("getChannels", true)
    )
    ...
    monkeyPatch(findModule("getMentionCount"), "getMentionCount", function () {
      ...
      if (
        ch &&
        ch.guild_id &&
        m.isMuted(ch.guild_id) &&
        m.isSuppressEveryoneEnabled(ch.guild_id)
      ) {
        return 0;
      }
